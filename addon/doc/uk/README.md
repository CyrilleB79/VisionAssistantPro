...
**Збереження документів в історії:** Зберігайте відкриті документи в списку історії; кешований текст OCR і дані для продовження також зберігаються.
...
- **Словник персонажів та керування серіалами:** додавайте, редагуйте, імпортуйте або керуйте іменами персонажів, їхніми фізичними характеристиками та ролями для кожного серіалу — ШІ автоматично зіставляє виявлених персонажів із вашим словником і додає нових у міру того, як ви аналізуєте нові епізоди. Ваші ручні нотатки завжди зберігаються і мають пріоритет над оновленнями від ШІ, тоді як фізичні характеристики персонажів залишаються актуальними у всіх епізодах.
...
- **Уніфікований каталог даних:** Усі файли даних додатка (історія, серіали, підписи, перебіг виконання OCR, кеші та журнали) зберігаються в єдиній папці `VisionAssistant` у каталозі налаштувань NVDA — це дозволяє підтримувати порядок і спрощує створення резервних копій вручну.
...
| **Control + H** | **Історія**            | Відкриває діалог «Історія», у якому показуються ваші попередні чати та документи, а також доступні фільтри за типом і опції «Видалити» та «Очистити». |
...
Ви також можете вибрати, що саме додаток має запам’ятовувати. Функції **«Зберегти чати в історії»** (вкладка «З’єднання») та **«Зберегти документи в історії»** (вкладка «Читач документів») початково увімкнені, і обидві їх можна ввімкнути або вимкнути в «Швидких налаштуваннях». Параметр щодо документів впливає лише на записи в історії — кешований текст, отриманий за допомогою OCR, та дані відновлення завжди зберігаються.
...
- **48-годинне кешування відеофайлів:** Відео, завантажені на Gemini, зберігаються в кеші протягом 48 годин! Ви можете повторно генерувати SRT- або MP3-файли для того самого відео без повторного завантаження — навіть після перезапуску NVDA. Кеш враховує ключ і автоматично анулюється у разі зміни вашого API-ключа.
...
- **Покращено збереження:** Під час збереження файлів SRT або MP3 діалог файлу тепер початково відкривається в папці вихідного відео, незалежно від того, чи відкрили ви відео через діалог файлу чи за допомогою Shift+V у Провіднику.
...
- **Стрілка вниз / вгору:** Коли курсор досягне останнього рядка сторінки, натисніть **вниз**, щоб перейти на наступну сторінку; натисніть **вгору** у верхній частині сторінки, щоб повернутися до попередньої.
...
- **Використання з веб-камери:** Поставте прапорець біля пункту **Використовувати веб-камеру** у діалозі Live Assistant, щоб надсилати ШІ зображення з камери замість екрана — запитуйте про фізичні об’єкти, роздруковані документи або оточення. Якщо ffmpeg ще не встановлено, при позначенні відповідного прапорця програма завантажиться один раз з вашого дозволу; ця опція вимкнена, якщо камера не виявлена або налаштування конфіденційності Windows блокують доступ до камери.
...
---
## Зміни від 01.10.2026

*   **Словник персонажів та керування серіалами**: Діалог «Аналіз відео» тепер містить потужну систему **Словника персонажів**! Додавайте, редагуйте, імпортуйте або керуйте іменами персонажів, їхніми фізичними характеристиками та ролями для кожної серії — ШІ автоматично зіставляє виявлених персонажів із вашим словником і додає нових у міру аналізу нових епізодів. Ваші ручні нотатки завжди зберігаються і мають пріоритет над оновленнями від ШІ, тоді як фізичні описи залишаються актуальними у всіх епізодах. Словник зберігається окремо для кожного серіалу та повторно використовується для кожного відео в цьому серіалі. У списку персонажів натисніть **F2**, щоб редагувати вибраного персонажа, та **Delete**, щоб видалити його.
*   **48-годинне кешування відеофайлів**: Відео, завантажені до Gemini, тепер кешуються протягом 48 годин! Ви можете повторно згенерувати SRT- або MP3-файли для того самого відео без повторного завантаження — навіть після перезапуску NVDA. Кеш враховує ключі і автоматично анулюється при зміні вашого API-ключа.
*   **Виявлення тиші на основі ШІ (Silero VAD)**: Розширений AD тепер використовує нейронну модель Silero VAD для точного виявлення тиші — розрізняючи природні паузи в діалозі від музики та фонового шуму. Модель завантажується автоматично під час першого використання (з вашого дозволу), так само як і ffmpeg та eSpeak.
*   **Відстеження першої появи персонажа**: ШІ тепер описує зовнішність кожного персонажа лише один раз — під час його першої появи у відео. Під час наступних появ використовуються лише імена, що усуває повторювані описи у різних сегментах, зберігаючи при цьому актуальність та природність розповіді.
*   **Уніфікований каталог даних**: Усі файли даних додатка (історія, серіали, підписи, перебіг виконання OCR, кеші та журнали) було переміщено в єдину папку `VisionAssistant` у вашому каталозі конфігурації NVDA — це дозволяє підтримувати порядок і спрощує створення резервних копій вручну.
*   **Покращений процес збереження відео**: під час збереження файлів SRT або MP3 діалог файлів тепер початково відкривається у папці вихідного відео, незалежно від того, чи відкриваєте ви відео через діалог файлів, чи за допомогою комбінації Shift+V у Провіднику.
*   **Видалення документів із кешованим текстом або без нього**: Діалог «Історія» (`Control + H`) тепер пропонує два способи видалення документа — натисніть клавішу Delete і виберіть **«Видалити лише з історії»** або **«Видалити з історії та кешованого тексту»**. Другий варіант очищає кешований текст OCR цього документа, тож під час наступного відкриття його буде перескановано з нуля — це ідеальний варіант після невдалого сканування. Прапорець **«Більше не запитувати»** запам’ятовує ваш вибір для майбутніх видалень. Дані про відновлення перерваної операції залишаються недоторканими.
*   **Інтеграція кешу OCR з урахуванням механізму в читачі документів**: кешований текст OCR тепер зберігається окремо для кожного механізму OCR, тому при зміні механізму OCR завжди відбувається повторне сканування за допомогою нового механізму, а не відтворення старого результату. При повторному відкритті документа знову з’являється діалог вибору діапазону сторінок (у якому вже вказано ваш останній вибір), що дозволяє миттєво використовувати вже відскановані сторінки та сканувати лише ті, яких бракує — кеш об’єднується сторінка за сторінкою, а не замінюється, тому кожен діапазон, який ви сканували, зберігається для подальшого використання.
*   **Відео з веб-камери для Live помічника**: У діалозі Live помічника тепер є прапорець **Використовувати веб-камеру**, щоб надсилати ШІ зображення з вашої веб-камери замість екрана — це ідеально підходить для запитань про фізичні об’єкти, документи або ваше оточення. Якщо ffmpeg ще не встановлено, позначення відповідного прапорця завантажить його (одноразово, з вашого дозволу). Ця функція вимкнена, якщо камера не виявлена або налаштування конфіденційності Windows блокують доступ до камери; у цьому випадку з’являється кнопка для відкриття налаштувань конфіденційності камери. Якщо камера увімкнена, але не передає кадри, проблема фіксується в журналі NVDA для діагностики, замість того щоб тихо повертатися до екрана.
*   **Вибір того, що зберігається (перемикачі історії)**: Дві нові функції дають вам контроль над вашими даними. **Зберігати чати в історії** (вкладка «З’єднання») визначає, чи додавати діалоги чату до списку історії, а **Зберігати документи в історії** (вкладка «Читач документів») — чи показувати там відкриті документи. Параметр для документів ніколи не зачіпає кешований текст OCR або дані про відновлення перерваних завдань, тому документи все одно відкриваються без повторного сканування, а незавершені видобування можна продовжувати. Обидва параметри початково ввімкнені, їх також можна вмикати та вимикати у «Швидких налаштуваннях».
*   **Виправлення помилок та покращення стабільності**: Виправлено зависання під час створення MP3-файлів при закритті діалогу в процесі виконання завдання, усунуто проблему в сховищі словника персонажів, додано належне повідомлення про помилки при невдалому кодуванні MP3, а також виправлено цикл очікування у читачі документів, щоб він враховував скасування користувачем. Чати тепер надійно зберігаються в «Історії» — включаючи результати користувацьких запитів та розмови, закриті за допомогою кнопки «Закрити», які раніше ніколи не зберігалися.
